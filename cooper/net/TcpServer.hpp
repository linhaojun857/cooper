...
public:
     ...
     */
    TcpServer(EventLoop* loop, const InetAddress& address, std::string name, bool reUseAddr = true,
              bool reUsePort = true);
    ~TcpServer();
     ...
     * @param ioLoops
     */
    void setIoLoops(const std::vector<cooper::EventLoop*>& ioLoops) {
     ...
     *
     * @return const cooper::InetAddress&
     */
    const cooper::InetAddress& address() const;
    ...
    [[deprecated("Use enableSSL(TLSPolicyPtr) instead")]] void enableSSL(
        const std::string& certPath, const std::string& keyPath, bool useOldTLS = false,
