...
static bool validatePeerCertificate(SSL* ssl, X509* cert, const std::string& hostname, bool allowBrokenChain,
                                    bool isServer) {
    assert(ssl != nullptr);
    assert(cert != nullptr);
...
static int serverSelectProtocol(SSL* ssl, const unsigned char** out, unsigned char* outlen, const unsigned char* in,
        ...
        if (ret == 1) {
            ...
            if (contextPtr_->isServer) {
                ...
                if (SSL_SESSION_is_resumable(session)) {
                    ...
                    if (reused == 0)
                        sessionManager.store(sniName_, conn_->peerAddr(), session, loop_);
                }
#endif
            }
            ...
            if (needCert) {
                if (cert) {
                    bool valid =
                        internal::validatePeerCertificate(ssl_, cert, policyPtr_->getHostname(),
                                                          policyPtr_->getAllowBrokenChain(), contextPtr_->isServer);
                    if (!valid) {
...
std::shared_ptr<TLSProvider> cooper::newTLSProvider(TcpConnection* conn, TLSPolicyPtr policy, SSLContextPtr ctx) {
    return std::make_shared<OpenSSLProvider>(conn, std::move(policy), std::move(ctx));
}

SSLContextPtr cooper::newSSLContext(const TLSPolicy& policy, bool isServer) {
