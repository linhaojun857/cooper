...
#include "cooper/net/Certificate.hpp"
#include "cooper/net/EventLoop.hpp"
#include "cooper/net/InetAddress.hpp"
#include "cooper/net/TLSPolicy.hpp"
...
public:
     ...
     * @param length
     */
    virtual void sendFile(const wchar_t* fileName, size_t offset = 0, size_t length = 0) = 0;
    /**
    ...
    [[deprecated("Use startEncryption(TLSPolicyPtr) instead")]] void startClientEncryption(
        std::function<void(const TcpConnectionPtr&)>&& callback, bool useOldTLS = false, bool validateCert = true,
        const std::string& hostname = "", const std::vector<std::pair<std::string, std::string>>& sslConfCmds = {}) {
...
SSLContextPtr newSSLContext(const TLSPolicy& policy, bool server);
