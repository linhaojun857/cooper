...
public:
     ...
     * @param microSecond
     */
    Date(unsigned int year, unsigned int month, unsigned int day, unsigned int hour = 0, unsigned int minute = 0,
         unsigned int second = 0, unsigned int microSecond = 0);
    ...
    static int64_t timezoneOffset() {
        static int64_t offset =
            -(Date::fromDbStringLocal("1970-01-03 00:00:00").secondsSinceEpoch() - 2LL * 3600LL * 24LL);
        return offset;
    }
    ...
    /**
     * @brief From DB string to cooper local time zone.
     *
     * Inverse of toDbStringLocal()
     */
    static Date fromDbStringLocal(const std::string& datetime);
    /**
     * @brief From DB string to cooper UTC time.
     *
